<!-- src/views/admin/partenaires/Liste.vue -->
...
const rejectPartenaire = ref(null);
...
async function fetchPartenaires() {
    ...
    try {
        ...
        const response = await apiClient.get('/admin/partenaires', { params });
        partenaires.value = response.data.data || [];
    } catch (err) { error.value = 'Impossible de charger les partenaires.'; }
...
function canEdit(partenaire) {
    if (authStore.isPresident) return true;
    if (authStore.isSecretaire && partenaire.auteur_id === currentUser.value?.id && ['brouillon', 'rejete'].includes(partenaire.statut)) return true;
    return false;
}
function canSubmit(partenaire) { return authStore.isSecretaire && partenaire.auteur_id === currentUser.value?.id && ['brouillon', 'rejete'].includes(partenaire.statut); }
function canValidate(partenaire) { return authStore.isPresident && partenaire.statut === 'en_attente'; }
function canReject(partenaire) { return authStore.isPresident && partenaire.statut === 'en_attente'; }
function canDelete(partenaire) { return authStore.isPresident; }

async function handleSubmit(partenaire) { try { await apiClient.post(`/admin/partenaires/${partenaire.id}/submit`); await fetchPartenaires(); } catch (err) { alert('Erreur lors de la soumission.'); } }
async function handleValidate(partenaire) { try { await apiClient.post(`/admin/partenaires/${partenaire.id}/validate`); await fetchPartenaires(); } catch (err) { alert('Erreur lors de la validation.'); } }
function openRejectModal(partenaire) { rejectPartenaire.value = partenaire; motifRejet.value = ''; rejectError.value = ''; showRejectModal.value = true; }
function closeRejectModal() { showRejectModal.value = false; rejectPartenaire.value = null; motifRejet.value = ''; rejectError.value = ''; }
async function handleReject() {
    ...
    try { await apiClient.post(`/admin/partenaires/${rejectPartenaire.value.id}/reject`, { motif_rejet: motifRejet.value }); closeRejectModal(); await fetchPartenaires(); }
    catch (err) { rejectError.value = err.response?.data?.errors?.motif_rejet?.[0] || 'Erreur lors du rejet.'; }
    finally { isRejecting.value = false; }
}
async function handleDelete(partenaire) {
    if (!confirm(`Supprimer le partenaire "${partenaire.nom}" ?`)) return;
    try { await apiClient.delete(`/admin/partenaires/${partenaire.id}`); await fetchPartenaires(); } catch (err) { alert('Erreur lors de la suppression.'); }
}
    ...
    <div class="liste-page">
        <div class="page-top">
            ...
            <div class="page-actions">
                <select v-model="filtreStatut" class="filter-select" @change="fetchPartenaires">
                    <option v-for="s in statuts" :key="s.value" :value="s.value">{{ s.label }}</option>
                </select>
                <button class="btn-primary" @click="router.push('/admin/partenaires/nouveau')">+ Nouveau partenaire</button>
            </div>
        </div>

        <div v-if="isLoading" class="loading">Chargement...</div>
        <div v-else-if="error" class="error">{{ error }}</div>
        <div v-else-if="partenaires.length === 0" class="empty">Aucun partenaire trouvé.</div>
            ...
            <table class="data-table">
                ...
                <tbody>
                    <tr v-for="partenaire in partenaires" :key="partenaire.id">
                        <td><img v-if="partenaire.logo" :src="`http://127.0.0.1:8000/storage/${partenaire.logo}`" :alt="partenaire.nom" class="logo-thumb" /></td>
                        <td class="cell-title">
                            {{ partenaire.nom }}
                            <div v-if="partenaire.statut === 'rejete' && partenaire.motif_rejet" class="motif-rejet">Motif : {{ partenaire.motif_rejet }}</div>
                        </td>
                        <td>{{ partenaire.lien_site || '-' }}</td>
                        <td><span class="statut-badge" :class="getStatutClass(partenaire.statut)">{{ getStatutLabel(partenaire.statut) }}</span></td>
                        <td>{{ partenaire.auteur?.name || '-' }}</td>
                        <td class="cell-actions">
                            <button v-if="canEdit(partenaire)" class="btn-action btn-action--edit" @click="router.push(`/admin/partenaires/${partenaire.id}/modifier`)" title="Modifier">✏️</button>
                            <button v-if="canSubmit(partenaire)" class="btn-action btn-action--submit" @click="handleSubmit(partenaire)" title="Soumettre">📤</button>
                            <button v-if="canValidate(partenaire)" class="btn-action btn-action--validate" @click="handleValidate(partenaire)" title="Valider">✅</button>
                            <button v-if="canReject(partenaire)" class="btn-action btn-action--reject" @click="openRejectModal(partenaire)" title="Rejeter">❌</button>
                            <button v-if="canDelete(partenaire)" class="btn-action btn-action--delete" @click="handleDelete(partenaire)" title="Supprimer">🗑️</button>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <Teleport to="body">
            <div v-if="showRejectModal" class="modal-overlay" @click.self="closeRejectModal">
                <div class="modal">
                    <h3>Rejeter le partenaire</h3>
                    <p class="modal-subtitle">{{ rejectPartenaire?.nom }}</p>
...
.logo-thumb { width: 40px; height: 40px; object-fit: contain; border-radius: $radius-sm; }
.cell-title { max-width: 200px; font-weight: $font-weight-medium; }
