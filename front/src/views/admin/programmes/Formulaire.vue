<!-- src/views/admin/programmes/Formulaire.vue -->
...
async function loadProgramme() {
    ...
    error.value = '';
    try {
        const response = await apiClient.get(`/admin/programmes/${route.params.id}`);
        form.value.titre = response.data.titre;
        form.value.description = response.data.description;
    } catch (err) {
        error.value = 'Impossible de charger le programme.';
    } finally {
...
async function handleSubmit() {
    ...
    try {
        if (isEdit.value) {
            ...
            await apiClient.post(`/admin/programmes/${route.params.id}`, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
        } else {
            await apiClient.post('/admin/programmes', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
        }
...
<template>
    <div class="formulaire-page">
        <div class="page-top">
            ...
            <button class="btn-back" @click="router.push('/admin/programmes')">← Retour à la liste</button>
        </div>
        ...
        <form v-else class="form" @submit.prevent="handleSubmit">
            ...
            <div class="form-group">
                ...
                <input id="titre" v-model="form.titre" type="text" placeholder="Titre du programme" :class="{ 'input-error': getFieldError('titre') }" />
                <span v-if="getFieldError('titre')" class="field-error">{{ getFieldError('titre') }}</span>
            </div>
            <div class="form-group">
                <label for="description">Description *</label>
                <textarea id="description" v-model="form.description" rows="12" placeholder="Description du programme..." :class="{ 'input-error': getFieldError('description') }"></textarea>
            ...
            <div class="form-actions">
                <button type="button" class="btn-cancel" @click="router.push('/admin/programmes')" :disabled="isSubmitting">Annuler</button>
                <button type="submit" class="btn-submit" :disabled="isSubmitting">{{ isSubmitting ? 'Enregistrement...' : (isEdit ? 'Mettre à jour' : 'Créer le programme') }}</button>
            </div>
        </form>
    </div>
</template>
...
.form-group { display: flex; flex-direction: column; gap: $spacing-xs;
    ...
    input[type="text"], textarea { padding: $spacing-sm $spacing-md; border: 1px solid $color-border; border-radius: $radius-md; font-family: $font-family-base; font-size: $font-size-base; color: $color-text; outline: none;
